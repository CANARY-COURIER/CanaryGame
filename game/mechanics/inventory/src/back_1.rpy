...
#         # Class variables 
#         name_tag="dragged_"
#         scale_intervals = 0.025
#         snap_ret_dur = 2.0  # snap_return_duration
...
#             scaled_slot_width = self.width * self.auto_scaled_value
#             scaled_slot_height = self.height * self.auto_scaled_value            
...
#             self.drag_widget.snap(self.origin_x, self.origin_y, self.snap_assign_dur)
...
#                 self.handle_dragging_scale(trans)
#             elif self.should_resize_to_origin:
#                 # Handle resizing back to the original scale
#                 self.resize_to_origin(trans, st, at)
#             else:
#                 # Maintain the current auto-scaled value
#                 trans.zoom = self.auto_scaled_value
#             return None
            
#         def handle_dragging_scale(self, trans):
...
#             scale_factor = compute_scale_factor(slot, self, self.padding_factor)
#             self.auto_scaled_value = scale_factor * self.default_scale
            
#         def resize_to_origin(self, trans, st, at):
#             """Smoothly resizes the item based on distance to original position and adjusts scaling accordingly."""
#             original_distance = ((self.origin_x - self.drag_widget.last_x) ** 2 + (self.origin_y - self.drag_widget.last_y) ** 2) ** 0.5
#             print("Distance from origin when dropped : ", original_distance)
#             # Calculate scaling ratio based on distance to origin.
#             if original_distance > self.origin_threshold:
#                 self.auto_scaled_value = ### scale ratio of 0 - original scale, to current distance - current scale
#             else:
...
#             """Returns the original distance between the item and its starting position."""
#             return ((self.origin_x - self.drag_widget.last_x) ** 2 + (self.origin_y - self.drag_widget.last_y) ** 2) ** 0.5
